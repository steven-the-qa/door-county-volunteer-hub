...
| F2 | As a visitor I only ever see **active, verified, currently-relevant** opportunities | Entries hidden automatically when `status != "open"`, `verified != true`, `postedDate` is in the future, or (if `expiresDate` is set) it is in the past | Client-side filter at render time using the browser's current date |
